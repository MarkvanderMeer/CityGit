...
def log(refs, graph):
    ...
    def processor(citymodel):
        command = cjv_commands.LogCommand(citymodel, refs, graph)
        command.execute()
    return processor
...
def checkout(ref, output, objectid_property, no_objectid):
    ...
    def processor(citymodel):
        command = cjv_commands.CheckoutCommand(citymodel, ref, output)
...
def diff(dest_ref, source_ref):
    ...
    def processor(citymodel):
        command = cjv_commands.DiffCommand(citymodel, dest_ref, source_ref)
        command.execute()
    return processor
...
def rehash(context, output):
    ...
    def processor(citymodel):
        command = cjv_commands.RehashCommand(citymodel, output)
        command.execute()
    return processor
...
def commit(context, new_version, ref, author, message, output):
    ...
    def processor(citymodel):
        command = cjv_commands.CommitCommand(citymodel,
                                         new_citymodel,
                                         ref,
                                         author,
                                         message)
...
def branch(context, delete, list_branches, branch_name, ref, output):
    ...
    def delete_processor(citymodel):
        command = cjv_commands.BranchDeleteCommand(citymodel, branch_name, output)
        command.execute()

    def create_processor(citymodel):
        command = cjv_commands.BranchCommand(citymodel, ref, branch_name, output)
        command.execute()
...
def merge(context, source_branch, dest_branch, author, output):
    ...
    def processor(citymodel: 'VersionedCityJSON'):
        command = cjv_commands.MergeBranchesCommand(citymodel,
                                                source_branch,
                                                dest_branch,
                                                author,
                                                output)
